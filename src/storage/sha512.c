...
#define __le32_to_cpus(x) htonl(x)
#define __cpu_to_le32s(x) ntohl(x)
...
void calculateHash(char* filename,
                   char* algname,
                   unsigned int hashLength,
                   void (*init)(void*),
                   void (*update)(void*, const u8*, unsigned int),
                   void (*final)(void*, u8*))
{
   char    ctx[1024];
   u8      hash[1024];
   ...
   if(fh!=NULL) {
      while(!feof(fh)) {
         count = fread(&data, 1, sizeof(data), fh);
         if(count > 0) {
            update(&ctx, data, count);
         }
      }
      fclose(fh);
   }
...
int main(int argc, char** argv)
{
   ...
   for(i = 1;i < argc;i++) {
      filename = argv[i];
      printf("FILE: %s\n", filename);
      calculateHash(filename, "SHA-512", SHA512_DIGEST_SIZE,
                    sha512_init, sha512_update, sha512_final);
      calculateHash(filename, "SHA-384", SHA384_DIGEST_SIZE,
                    sha384_init, sha512_update, sha384_final);
      calculateHash(filename, "SHA-1",   SHA1_DIGEST_SIZE,
                    sha1_init, sha1_update, sha1_final);
      calculateHash(filename, "MD-5",    MD5_DIGEST_SIZE,
