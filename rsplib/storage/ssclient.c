...
#define MAX_PU_TRANSPORTADDRESSES 64

/* ###### Main program ################################################### */
int main(int argc, char** argv)
{
   const char*                  poolHandle = "ScriptingPool";
   char                         localAddressBuffer[sizeof(union sockaddr_union) * MAX_PU_TRANSPORTADDRESSES];
   ...
   for(i = 1;i < argc;i++) {
      if(rsp_initarg(&info, argv[i])) {
         ...
         localAddresses = 0;
         addressPtr     = localAddressSet;
         addressString  = (char*)&argv[i][7];
         while(localAddresses < MAX_PU_TRANSPORTADDRESSES) {
   ...
   if(rsp_bind(sd, localAddressSet, localAddresses) < 0) {
      newLogLine(stderr);
