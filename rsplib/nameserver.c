                               ...
                               FILE*       fd)
{
   ...
   fprintf(fd, "   - Takeover for $%08x (expiry in %lldµs)\n",
...
#ifdef ENABLE_CSP
static size_t nameServerGetReportFunction(
                 void*                              userData,
                                 ...
                                 )
{
   ...
   if(nameServer != NULL) {
      ...
      if(nameServer->CSPReportInterval > 0) {
         memcpy(&nameServer->CSPReportAddress,
                cspReportAddress,
                sizeof(nameServer->CSPReportAddress));
         cspReporterNew(&nameServer->CSPReporter, &nameServer->StateMachine,
                        CID_COMPOUND(CID_GROUP_NAMESERVER, nameServer->ServerID),
                        &nameServer->CSPReportAddress,
                        nameServer->CSPReportInterval,
                        nameServerGetReportFunction,
                        nameServer);
      }
#endif
   }
...
#ifdef ENABLE_CSP
      cspReporterDelete(&nameServer->CSPReporter);
#endif
                                    ...
                                    void*              userData)
{
   ...
   while((peerListNode != NULL) &&
      ...
      if(peerListNode->TimerCode == PLNT_MAX_TIME_LAST_HEARD) {
         ...
         fprintf(stdlog, " not head since MaxTimeLastHeard=%lluµs -> requesting immediate PeerPresence\n",
         ...
         fputs("Peer ", stdlog);
         ST_CLASS(peerListNodePrint)(peerListNode, stdlog, PLPO_FULL);
         fprintf(stdlog, " did not answer in MaxTimeNoResponse=%lluµs -> removing it\n",
                 nameServer->PeerMaxTimeLastHeard);
         LOG_END
...
int main(int argc, char** argv)
{
   ...
   if(cspReportInterval > 0) {
      ...
      puts("");
      printf("CSP Report Interval:    %lldµs\n", cspReportInterval);
   }
#endif

   puts("\nASAP Parameters:");
   printf("   Max Bad PE Reports:                          %u\n",     (unsigned int)nameServer->MaxBadPEReports);
   printf("   Server Announce Cycle:                       %lldµs\n", nameServer->ServerAnnounceCycle);
   printf("   Endpoint Monitoring SCTP Heartbeat Interval: %lldµs\n", nameServer->EndpointMonitoringHeartbeatInterval);
   printf("   Endpoint Keep Alive Transmission Interval:   %lldµs\n", nameServer->EndpointKeepAliveTransmissionInterval);
   printf("   Endpoint Keep Alive Timeout Interval:        %lldµs\n", nameServer->EndpointKeepAliveTimeoutInterval);
   puts("ENRP Parameters:");
   printf("   Peer Heartbeat Cylce:                        %lldµs\n", nameServer->PeerHeartbeatCycle);
   printf("   Peer Max Time Last Heard:                    %lldµs\n", nameServer->PeerMaxTimeLastHeard);
   printf("   Peer Max Time No Response:                   %lldµs\n", nameServer->PeerMaxTimeNoResponse);
   printf("   Mentor Hunt Timeout:                         %lldµs\n", nameServer->MentorHuntTimeout);
   printf("   Takeover Expiry Interval:                    %lldµs\n", nameServer->TakeoverExpiryInterval);
   puts("");
