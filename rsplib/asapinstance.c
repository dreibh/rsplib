/*
 *  $Id: asapinstance.c,v 1.25 2004/11/09 19:03:22 dreibh Exp $
 *
...
static void asapInstanceConfigure(struct ASAPInstance* asapInstance, struct TagItem* tags)
{
   ...
   fprintf(stdlog, "nameserver.request.timeout     = %Luµs\n", asapInstance->NameServerRequestTimeout);
   fprintf(stdlog, "nameserver.response.timeout    = %Luµs\n", asapInstance->NameServerResponseTimeout);
   fprintf(stdlog, "cache.elementtimeout           = %Luµs\n", asapInstance->CacheElementTimeout);
   LOG_END
}
               ...
               void*              userData)
{
   ...
   if(fd == asapInstance->NameServerSocket) {
